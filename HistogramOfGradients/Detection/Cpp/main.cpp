...
int main(int argc, char** argv) {
  ...
  auto trainData = TrainData::create(train_data, ROW_SAMPLE, labels);
  svm->train(trainData);

  clog << "...[done]" << endl;
  std::vector<int> dataOut;
  clog << "Percent of missclassified samples: "
       << svm->calcError(trainData, false, dataOut) << endl;
  ...
  if (train_twice) {
    ...
    for (size_t i = 0; i < neg_lst.size(); i++) {
      if (neg_lst[i].cols >= pos_image_size.width &&
          neg_lst[i].rows >= pos_image_size.height)
