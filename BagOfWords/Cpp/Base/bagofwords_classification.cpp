...
#error "OpenCV was built without ml module"
...
static void help(char** argv) {
  cout
      << "\nThis program shows how to read in, train on and produce test "
         "results for the PASCAL VOC (Visual Object Challenge) data. \n"
      ...
      << "       or:  \n"
      << " ./" << argv[0]
      << " [VOC path] [result directory] [feature detector] [descriptor "
         "extractor] [descriptor matcher] \n"
      << "\n"
      << "Input parameters: \n"
      << "[VOC path]             Path to Pascal VOC data (e.g. "
         "/home/my/VOCdevkit/VOC2010). Note: VOC2007-VOC2010 are supported. \n"
      << "[result directory]     Path to result diractory. Following folders "
         "will be created in [result directory]: \n"
      << "                         bowImageDescriptors - to store image "
         "descriptors, \n"
      << "                         svms - to store trained svms, \n"
      << "                         plots - to store files for plots creating. "
         "\n"
      << "[feature detector]     Feature detector name (e.g. KAZE, SIFT) - "
         "see createFeatureDetector() function in detectors.cpp \n"
      << "                         Currently 12/2021, this is KAZE, SIFT \n"
      << "[descriptor extractor] Descriptor extractor name (e.g. SURF, SIFT) - "
         "see createDescriptorExtractor() function in descriptors.cpp \n"
      << "                         Currently 12/2021, this is KAZE, SIFT \n"
      << "[descriptor matcher]   Descriptor matcher name (e.g. BruteForce) - "
         "see createDescriptorMatcher() function in matchers.cpp \n"
      << "                         Currently 12/2021, this is BruteForce, "
         "BruteForce-L1, FlannBased, BruteForce-Hamming, BruteForce-HammingLUT "
         "\n"
      << "\n";
}
...
// used by getDetectorGroundTruth to sort a two dimensional list of floats in
// descending order
class ObdScoreIndexSorter {
 public:
  ...
  int image_idx;
  int obj_idx;
  bool operator<(const ObdScoreIndexSorter& compare) const {
...
class VocData {
 public:
  ...
  ~VocData() {}
  /* functions for returning classification/object data for multiple images
   * given an object class */
  void getClassImages(const string& obj_class, const ObdDatasetType dataset,
                      vector<ObdImage>& images, vector<char>& object_present);
  void getClassObjects(const string& obj_class, const ObdDatasetType dataset,
  ...
  ObdImage getObjects(const string& obj_class, const string& id,
                      vector<ObdObject>& objects,
                      vector<VocObjectData>& object_data, VocGT& ground_truth);
  /* functions for returning the ground truth (present/absent) for groups of
   * images */
  void getClassifierGroundTruth(const string& obj_class,
                                const vector<ObdImage>& images,
                                vector<char>& ground_truth);
  void getClassifierGroundTruth(const string& obj_class,
                                const vector<string>& images,
                                vector<char>& ground_truth);
  int getDetectorGroundTruth(const string& obj_class,
                             const ObdDatasetType dataset,
                             const vector<ObdImage>& images,
                             const vector<vector<Rect> >& bounding_boxes,
                             const vector<vector<float> >& scores,
                             vector<vector<char> >& ground_truth,
                             vector<vector<char> >& detection_difficult,
                             bool ignore_difficult = true);
  /* functions for writing VOC-compatible results files */
  void writeClassifierResultsFile(const string& out_dir,
                                  const string& obj_class,
                                  const ObdDatasetType dataset,
                                  const vector<ObdImage>& images,
                                  const vector<float>& scores,
                                  const int competition = 1,
                                  const bool overwrite_ifexists = false);
  /* functions for calculating metrics from a set of classification/detection
   * results */
  string getResultsFilename(const string& obj_class, const VocTask task,
                            const ObdDatasetType dataset,
                            const int competition = -1, const int number = -1);
  void calcClassifierPrecRecall(const string& obj_class,
                                const vector<ObdImage>& images,
                                const vector<float>& scores,
                                vector<float>& precision, vector<float>& recall,
                                float& ap, vector<size_t>& ranking);
  void calcClassifierPrecRecall(const string& obj_class,
                                const vector<ObdImage>& images,
                                const vector<float>& scores,
                                vector<float>& precision, vector<float>& recall,
                                float& ap);
  void calcClassifierPrecRecall(const string& input_file,
                                vector<float>& precision, vector<float>& recall,
                                float& ap, bool outputRankingFile = false);
  /* functions for calculating confusion matrices */
  void calcClassifierConfMatRow(const string& obj_class,
                                const vector<ObdImage>& images,
                                const vector<float>& scores,
                                const VocConfCond cond, const float threshold,
                                vector<string>& output_headers,
                                vector<float>& output_values);
  void calcDetectorConfMatRow(
      const string& obj_class, const ObdDatasetType dataset,
      const vector<ObdImage>& images, const vector<vector<float> >& scores,
  ...
  void savePrecRecallToGnuplot(
      const string& output_file, const vector<float>& precision,
      const vector<float>& recall, const float ap,
      const string title = string(),
      const VocPlotType plot_type = CV_VOC_PLOT_SCREEN);
  /* functions for reading in result/ground truth files */
  void readClassifierGroundTruth(const string& obj_class,
                                 const ObdDatasetType dataset,
                                 vector<ObdImage>& images,
  ...
  void readClassifierResultsFile(const std::string& input_file,
                                 vector<ObdImage>& images,
                                 vector<float>& scores);
  void readDetectorResultsFile(const string& input_file,
                               vector<ObdImage>& images,
 ...
 protected:
  ...
  void readClassifierGroundTruth(const string& filename,
                                 vector<string>& image_codes,
                                 vector<char>& object_present);
  void readClassifierResultsFile(const string& input_file,
                                 vector<string>& image_codes,
                                 vector<float>& scores);
  void readDetectorResultsFile(const string& input_file,
                               vector<string>& image_codes,
                               vector<vector<float> >& scores,
                               vector<vector<Rect> >& bounding_boxes);
  void extractVocObjects(const string filename, vector<ObdObject>& objects,
                         vector<VocObjectData>& object_data);
  string getImagePath(const string& input_str);

  void getClassImages_impl(const string& obj_class, const string& dataset_str,
                           vector<ObdImage>& images,
                           vector<char>& object_present);
  void calcPrecRecall_impl(const vector<char>& ground_truth,
                           const vector<float>& scores,
                           vector<float>& precision, vector<float>& recall,
                           float& ap, vector<size_t>& ranking,
                           int recall_normalization = -1);

  // test two bounding boxes to see if they meet the overlap criteria defined in
  // the VOC documentation
  float testBoundingBoxesForOverlap(const Rect detection,
                                    const Rect ground_truth);
  // extract class and dataset name from a VOC-standard classification/detection
  // results filename
  void extractDataFromResultsFilename(const string& input_file,
                                      string& class_name, string& dataset_name);
  // get classifier ground truth for a single image
  bool getClassifierGroundTruthImage(const string& obj_class, const string& id);

  // utility functions
  void getSortOrder(const vector<float>& values, vector<size_t>& order,
  ...
  string checkFilenamePathsep(const string filename,
                              bool add_trailing_slash = false);
  void convertImageCodesToObdImages(const vector<string>& image_codes,
                                    vector<ObdImage>& images);
  int extractXMLBlock(const string src, const string tag, const int searchpos,
                      string& tag_contents);
  // utility sorter
  struct orderingSorter {
    bool operator()(std::pair<size_t, vector<float>::const_iterator> const& a,
                    std::pair<size_t, vector<float>::const_iterator> const& b) {
...
// Return the classification ground truth data for all images of a given VOC
// object class
//--------------------------------------------------------------------------------------
// INPUTS:
// - obj_class          The VOC object class identifier string
// - dataset            Specifies whether to extract images from the training or
// test set
// OUTPUTS:
// - images             An array of ObdImage containing info of all images
// extracted from the ground truth file
// - object_present     An array of bools specifying whether the object defined
// by 'obj_class' is present in each image or not
// NOTES:
// This function is primarily useful for the classification task, where only
// whether a given object is present or not in an image is required, and not
// each object instance's position etc.
void VocData::getClassImages(const string& obj_class,
                             const ObdDatasetType dataset,
                             vector<ObdImage>& images,
                             vector<char>& object_present) {
  string dataset_str;
  // generate the filename of the classification ground-truth textfile for the
  // object class
...
void VocData::getClassImages_impl(const string& obj_class,
                                  const string& dataset_str,
                                  vector<ObdImage>& images,
                                  vector<char>& object_present) {
  // generate the filename of the classification ground-truth textfile for the
  // object class
...
// test set
// OUTPUTS:
// - images             An array of ObdImage containing info of all images in
// chosen dataset (tag, path etc.)
// - objects            Contains the extended object info (bounding box etc.)
// for each object instance in each image
// - object_data        Contains VOC-specific extended object info (marked
// difficult etc.)
// - ground_truth       Specifies whether there are any difficult/non-difficult
// instances of the current
//                          object class within each image
// NOTES:
// This function returns extended object information in addition to the
// absent/present classification data returned by getClassImages. The objects
// returned for each image in the 'objects' array are of all object classes
// present in the image, and not just the class defined by 'obj_class'.
// 'ground_truth' can be used to determine quickly whether an object instance of
// the given class is present in an image or not.
void VocData::getClassObjects(const string& obj_class,
  ...
  getClassObjects(obj_class, dataset, images, objects, object_data,
                  ground_truth);
}
...
void VocData::getClassObjects(const string& obj_class,
  ...
  for (size_t i = 0; i < image_codes.size(); ++i) {
    ObdImage image = getObjects(obj_class, image_codes[i], image_objects,
                                image_object_data, image_gt);
...
//-----------------------------------------------------------------------------
// INPUTS:
// - id                 VOC Dataset unique identifier (string code in form
// YYYY_XXXXXX where YYYY is the year)
// OUTPUTS:
// - obj_class (*3)     Specifies the object class to use to resolve
// 'ground_truth'
// - objects            Contains the extended object info (bounding box etc.)
// for each object in the image
// - object_data (*2,3) Contains VOC-specific extended object info (marked
// difficult etc.)
// - ground_truth (*3)  Specifies whether there are any difficult/non-difficult
// instances of the current
...
//  * One returns the same as (2) plus the ground_truth flag. This also requires
//  an extra input obj_class [3]
...
ObdImage VocData::getObjects(const string& obj_class, const string& id,
                             vector<ObdObject>& objects,
                             vector<VocObjectData>& object_data,
                             VocGT& ground_truth) {
  ...
  // pregenerate a flag to indicate whether the current class is present or not
  // in the image
  ...
  for (size_t j = 0; j < objects.size(); ++j) {
    if (objects[j].object_class == obj_class) {
      if (object_data[j].difficult == false) {
        // if at least one non-difficult example is present, this flag is always
        // set to CV_VOC_GT_PRESENT
        ground_truth = CV_VOC_GT_PRESENT;
        break;
      } else {
        // set if at least one object instance is present, but it is marked
        // difficult
...
// Return ground truth data for the presence/absence of a given object class in
// an arbitrary array of images
//---------------------------------------------------------------------------------------------------------
// INPUTS:
// - obj_class          The VOC object class identifier string
// - images             An array of ObdImage OR strings containing the images
// for which ground truth
//                          will be computed
// OUTPUTS:
// - ground_truth       An output array indicating the presence/absence of
// obj_class within each image
void VocData::getClassifierGroundTruth(const string& obj_class,
                                       const vector<ObdImage>& images,
                                       vector<char>& ground_truth) {
  ...
  for (vector<ObdImage>::const_iterator it = images.begin(); it != images.end();
       ++it, ++gt_it) {
...
void VocData::getClassifierGroundTruth(const string& obj_class,
  ...
  vector<VocObjectData> object_data;
  vector<char>::iterator gt_it = ground_truth.begin();
  for (vector<string>::const_iterator it = images.begin(); it != images.end();
       ++it, ++gt_it) {
...
// INPUTS:
// - obj_class          The VOC object class identifier string
// - images             An array of ObdImage containing the images for which
// ground truth
//                          will be computed
// - bounding_boxes     A 2D input array containing the bounding box rects of
// the objects of
//                          obj_class which were detected in each image
// OUTPUTS:
// - ground_truth       A 2D output array indicating whether each object
// detection was accurate
//                          or not
// - detection_difficult A 2D output array indicating whether the detection
// fired on an object
//                          marked as 'difficult'. This allows it to be ignored
//                          if necessary (the voc documentation specifies
//                          objects marked as difficult have no effects on the
//                          results and are effectively ignored)
// - (ignore_difficult) If set to true, objects marked as difficult will be
// ignored when returning
//                          the number of hits for p-r normalization (default =
//                          true)
// RETURN VALUE:
//                      Returns the number of object hits in total in the gt to
//                      allow proper normalization
//                          of a p-r curve
// NOTES:
// As stated in the VOC documentation, multiple detections of the same object in
// an image are considered FALSE detections e.g. 5 detections of a single object
// is counted as 1 correct detection and 4 false detections - it is the
// responsibility of the participant's system to filter multiple detections from
// its output
int VocData::getDetectorGroundTruth(
    const string& obj_class, const ObdDatasetType dataset,
    const vector<ObdImage>& images, const vector<vector<Rect> >& bounding_boxes,
    const vector<vector<float> >& scores, vector<vector<char> >& ground_truth,
    vector<vector<char> >& detection_difficult, bool ignore_difficult) {
  int recall_normalization = 0;

  /* first create a list of indices referring to the elements of bounding_boxes
   * and scores in descending order of scores */
  ...
  /* preload object ground truth bounding box data */
  {
    ...
    for (size_t image_idx = 0; image_idx < images.size(); ++image_idx) {
      ...
      getObjects(images[image_idx].id, img_objects_all[image_idx],
                 img_object_data_all[image_idx]);
      /* meanwhile, also set length of target ground truth + difficult vector to
       * same as number of object detections (2nd dimension) */
    ...
    for (size_t image_idx = 0; image_idx < images.size(); ++image_idx) {
      for (size_t obj_idx = 0; obj_idx < img_objects_all[image_idx].size();
           ++obj_idx) {
        if (img_objects_all[image_idx][obj_idx].object_class == obj_class) {
          ...
          img_object_data[image_idx].push_back(
              img_object_data_all[image_idx][obj_idx]);
        }
      }
  ...
  /* calculate the total number of objects in the ground truth for the current
   * dataset */
  {
    ...
    for (size_t image_idx = 0; image_idx < gt_images.size(); ++image_idx) {
      ...
      for (size_t obj_idx = 0; obj_idx < gt_img_objects.size(); ++obj_idx) {
        if (gt_img_objects[obj_idx].object_class == obj_class) {
          if ((gt_img_object_data[obj_idx].difficult == false) ||
  ...
  int printed_count = 0;
#endif
  /* now iterate through detections in descending order of score, assigning to
   * ground truth bounding boxes if possible */
  for (size_t detect_idx = 0; detect_idx < sorted_ids.size(); ++detect_idx) {
    ...
    //-- for each detected object iterate through objects present in the
    // bounding box ground truth --
    for (size_t gt_obj_idx = 0; gt_obj_idx < img_objects[im_idx].size();
         ++gt_obj_idx) {
      if (detected[im_idx][gt_obj_idx] == false) {
        // check if the detected object and ground truth object overlap by a
        // sufficient margin
        float ov = testBoundingBoxesForOverlap(
            bounding_boxes[im_idx][ob_idx],
            img_objects[im_idx][gt_obj_idx].boundingBox);
        if (ov != -1.0) {
          // if all conditions are met store the overlap score and index (as
          // objects are assigned to the highest scoring match)
          if (ov > maxov) {
            maxov = ov;
    ...
    //-- if a match was found, set the ground truth of the current object to
    // true --
    if (maxov != -1.0) {
      ...
      // remove the ground truth object so it doesn't match with subsequent
      // detected objects
...
#ifdef PR_DEBUG
    if (printed_count < 10) {
      cout << printed_count << ": id=" << images[im_idx].id
           << ", score=" << scores[im_idx][ob_idx] << " (" << ob_idx << ") ["
           << bounding_boxes[im_idx][ob_idx].x << ","
           << bounding_boxes[im_idx][ob_idx].y << ","
           << bounding_boxes[im_idx][ob_idx].width +
                  bounding_boxes[im_idx][ob_idx].x
           << ","
           << bounding_boxes[im_idx][ob_idx].height +
                  bounding_boxes[im_idx][ob_idx].y
           << "] detected=" << ground_truth[im_idx][ob_idx]
           << ", difficult=" << detection_difficult[im_idx][ob_idx] << endl;
      ++printed_count;
      /* print ground truth */
      for (int gt_obj_idx = 0; gt_obj_idx < img_objects[im_idx].size();
           ++gt_obj_idx) {
        cout << "    GT: [" << img_objects[im_idx][gt_obj_idx].boundingBox.x
             << "," << img_objects[im_idx][gt_obj_idx].boundingBox.y << ","
             << img_objects[im_idx][gt_obj_idx].boundingBox.width +
                    img_objects[im_idx][gt_obj_idx].boundingBox.x
             << ","
             << img_objects[im_idx][gt_obj_idx].boundingBox.height +
                    img_objects[im_idx][gt_obj_idx].boundingBox.y
             << "]";
        if (gt_obj_idx == max_gt_obj_idx)
          cout << " <--- (" << maxov << " overlap)";
        cout << endl;
      }
    }
...
// - images             An array of ObdImage containing the images for which
// data will be saved to the result file
// - scores             A corresponding array of confidence scores given a query
// - (competition)      If specified, defines which competition the results are
// for (see VOC documentation - default 1)
// NOTES:
// The result file path and filename are determined automatically using
// m_results_directory as a base
void VocData::writeClassifierResultsFile(const string& out_dir,
                                         const string& obj_class,
                                         const ObdDatasetType dataset,
                                         const vector<ObdImage>& images,
                                         const vector<float>& scores,
                                         const int competition,
                                         const bool overwrite_ifexists) {
  CV_Assert(images.size() == scores.size());

  string output_file_base, output_file;
  if (dataset == CV_OBD_TRAIN) {
    output_file_base = out_dir + "/comp" + integerToString(competition) +
                       "_cls_" + m_train_set + "_" + obj_class;
  } else {
    output_file_base = out_dir + "/comp" + integerToString(competition) +
                       "_cls_" + m_test_set + "_" + obj_class;
  }
  ...
  if (result_file.is_open()) {
    ...
    string err_msg =
        "could not open classifier results file '" + output_file +
        "' for writing. Before running for the first time, a 'results' "
        "subdirectory should be created within the VOC dataset base directory. "
        "e.g. if the VOC data is stored in /VOC/VOC2010 then the path "
        "/VOC/results must be created.";
...
// INPUTS:
// - obj_class          The VOC object class identifier string
// - task               Specifies whether to generate a filename for the
// classification or detection task
// - dataset            Specifies whether working with the training or test set
// - (competition)      If specified, defines which competition the results are
// for (see VOC documentation
//                      default of -1 means this is set to 1 for the
//                      classification task and 3 for the detection task)
// - (number)           If specified and above 0, defines which of a number of
// duplicate results file produced for a given set of
//                      of settings should be used (this number will be added as
//                      a postfix to the filename)
// NOTES:
// This is primarily useful for returning the filename of a classification file
// previously computed using writeClassifierResultsFile for example when calling
// calcClassifierPrecRecall
string VocData::getResultsFilename(const string& obj_class, const VocTask task,
                                   const ObdDatasetType dataset,
                                   const int competition, const int number) {
  if ((competition < 1) && (competition != -1))
    CV_Error(Error::StsBadArg,
             "competition argument should be a positive non-zero number or -1 "
             "to accept the default");
  if ((number < 1) && (number != -1))
    CV_Error(Error::StsBadArg,
             "number argument should be a positive non-zero number or -1 to "
             "accept the default");
  ...
  stringstream ss;
  if (number < 1) {
    ss << "comp" << comp << "_" << task_type << "_" << dset << "_" << obj_class
       << ".txt";
  } else {
    ss << "comp" << comp << "_" << task_type << "_" << dset << "_" << obj_class
       << "_" << number << ".txt";
...
//--------------------------------------------
// INPUTS:
// - ground_truth       A vector of booleans determining whether the currently
// tested class is present in each input image
// - scores             A vector containing the similarity score for each input
// image (higher is more similar)
// OUTPUTS:
// - precision          A vector containing the precision calculated at each
// datapoint of a p-r curve generated from the result set
// - recall             A vector containing the recall calculated at each
// datapoint of a p-r curve generated from the result set
// - ap                The ap metric calculated from the result set
// - (ranking)          A vector of the same length as 'ground_truth' and
// 'scores' containing the order of the indices in both of
//                      these arrays when sorting by the ranking score in
//                      descending order
// NOTES:
// The result file path and filename are determined automatically using
// m_results_directory as a base
void VocData::calcClassifierPrecRecall(const string& obj_class,
                                       const vector<ObdImage>& images,
                                       const vector<float>& scores,
                                       vector<float>& precision,
                                       vector<float>& recall, float& ap,
...
//< Overloaded version which accepts VOC classification result file input
// instead of array of scores/ground truth > INPUTS:
// - input_file         The path to the VOC standard results file to use for
// calculating precision/recall
//                      If a full path is not specified, it is assumed this file
//                      is in the VOC standard results directory A VOC standard
//                      filename can be retrieved (as used by
//                      writeClassifierResultsFile) by calling
//                      getClassifierResultsFilename

void VocData::calcClassifierPrecRecall(const string& input_file,
                                       vector<float>& precision,
                                       vector<float>& recall, float& ap,
                                       bool outputRankingFile) {
  ...
  if (outputRankingFile) {
    ...
    vector<std::pair<size_t, vector<float>::const_iterator> > order(
        res_scores.size());

    size_t n = 0;
    for (vector<float>::const_iterator it = res_scores.begin();
         it != res_scores.end(); ++it, ++n)
    ...
    string scoregt_file_str = input_file_std1.substr(0, fnamestart + 1) +
                              "scoregt_" + class_name + ".txt";
    std::ofstream scoregt_file(scoregt_file_str.c_str());
    if (scoregt_file.is_open()) {
      for (size_t i = 0; i < res_scores.size(); ++i) {
        scoregt_file << res_image_codes[order[i].first] << " "
                     << res_scores[order[i].first] << " "
                     << res_ground_truth[order[i].first] << endl;
      }
      scoregt_file.close();
    } else {
      string err_msg =
          "could not open scoregt file '" + scoregt_file_str + "' for writing.";
      CV_Error(Error::StsError, err_msg.c_str());
    }
  }
  ...
  vector<size_t> ranking;
  calcPrecRecall_impl(res_ground_truth, res_scores, precision, recall, ap,
                      ranking);
}

//< Protected implementation of Precision-Recall calculation used by both
// calcClassifierPrecRecall and calcDetectorPrecRecall >

void VocData::calcPrecRecall_impl(const vector<char>& ground_truth,
                                  const vector<float>& scores,
                                  vector<float>& precision,
                                  vector<float>& recall, float& ap,
                                  vector<size_t>& ranking,
                                  int recall_normalization) {
  CV_Assert(ground_truth.size() == scores.size());

  // add extra element for p-r at 0 recall (in case that first retrieved is
  // positive)
  ...
  if (scoregt_file.is_open()) {
    for (int i = 0; i < scores.size(); ++i) {
      scoregt_file << scores[ranking[i]] << " " << ground_truth[ranking[i]]
                   << endl;
    }
    scoregt_file.close();
  }
...
#else
    recall_norm =
        (int)std::count_if(ground_truth.begin(), ground_truth.end(),
  ...
  for (size_t idx = 0; idx < ground_truth.size(); ++idx) {
    ...
    precision[idx + 1] =
        static_cast<float>(retrieved_hits) / static_cast<float>(idx + 1);
    recall[idx + 1] =
        static_cast<float>(retrieved_hits) / static_cast<float>(recall_norm);

    if (idx == 0) {
      // add further point at 0 recall with the same precision value as the
  ...
  if (m_sampled_ap == false) {
    ...
    for (vector<float>::iterator prec_it = precision.begin();
    ...
    for (size_t idx = 0; idx < (recall.size() - 1); ++idx) {
      ap += (recall[idx + 1] - recall[idx]) *
                precision_monot[idx + 1] +  // no need to take min of prec - is
                                            // monotonically decreasing
            0.5f * (recall[idx + 1] - recall[idx]) *
                std::abs(precision_monot[idx + 1] - precision_monot[idx]);
    }
  } else {
    // FOR BEFORE VOC2010 AP IS CALCULATED BY SAMPLING PRECISION AT RECALL
    // 0.0,0.1,..,1.0

    for (float recall_pos = 0.f; recall_pos <= 1.f; recall_pos += 0.1f) {
      // find iterator of the precision corresponding to the first recall >=
      // recall_pos
      ...
      /* if no recall >= recall_pos found, this level of recall is never reached
       * so stop adding to ap */
      if (recall_it == recall.end()) break;

      /* if the prec_it is valid, compute the max precision at this level of
       * recall or higher */
      vector<float>::iterator max_prec =
          std::max_element(prec_it, precision.end());
...
//------------------------------------
// INPUTS:
// - obj_class          The VOC object class identifier string for the confusion
// matrix row to compute
// - images             An array of ObdImage containing the images to use for
// the computation
// - scores             A corresponding array of confidence scores for the
// presence of obj_class in each image
// - cond               Defines whether to use a cut off point based on recall
// (CV_VOC_CCOND_RECALL) or score
//                      (CV_VOC_CCOND_SCORETHRESH) the latter is useful for
//                      classifier detections where positive values are positive
//                      detections and negative values are negative detections
// - threshold          Threshold value for cond. In case of
// CV_VOC_CCOND_RECALL, is proportion recall (e.g. 0.5).
//                      In the case of CV_VOC_CCOND_SCORETHRESH is the value
//                      above which to count results.
// OUTPUTS:
// - output_headers     An output vector of object class headers for the
// confusion matrix row
// - output_values      An output vector of values for the confusion matrix row
// corresponding to the classes
//                      defined in output_headers
// NOTES:
// The methodology used by the classifier version of this function is that true
// positives have a single unit added to the obj_class column in the confusion
// matrix row, whereas false positives have a single unit distributed in
// proportion between all the columns in the confusion matrix row corresponding
// to the objects present in the image.
void VocData::calcClassifierConfMatRow(
    const string& obj_class, const vector<ObdImage>& images,
    const vector<float>& scores, const VocConfCond cond, const float threshold,
  ...
  int target_idx;
  {
    vector<string>::iterator target_idx_it =
        std::find(output_headers.begin(), output_headers.end(), obj_class);
    /* if the target class can not be found, raise an exception */
    if (target_idx_it == output_headers.end()) {
      string err_msg = "could not find the target object class '" + obj_class +
                       "' in list of valid classes.";
      CV_Error(Error::StsError, err_msg.c_str());
    }
  ...
  if (cond == CV_VOC_CCOND_RECALL) {
    vector<char> ground_truth;
    /* in order to calculate the total number of relevant images for
       normalization of recall it's necessary to extract the ground truth for
  ...
  for (size_t image_idx = 0; image_idx < images.size(); ++image_idx) {
    ...
    /* if continuing for this iteration, increment the image counter for later
     * normalization */
    ++total_images;
    ...
    if (getClassifierGroundTruthImage(obj_class,
                                      images[ranking[image_idx]].id)) {
      // if the target class is present, assign fully to the target class
      // element in the confusion matrix row
      ...
      // if the target class is not present, add values to the confusion matrix
      // row in equal proportions to all objects present in the image
      for (size_t obj_idx = 0; obj_idx < img_objects.size(); ++obj_idx) {
        // find the index of the currently considered object
        vector<string>::iterator class_idx_it =
            std::find(output_headers.begin(), output_headers.end(),
                      img_objects[obj_idx].object_class);
        // if the class name extracted from the ground truth file could not be
        // found in the list of available classes, raise an exception
        if (class_idx_it == output_headers.end()) {
          string err_msg = "could not find object class '" +
                           img_objects[obj_idx].object_class +
        ...
        /* convert iterator to index */
        int class_idx =
            (int)std::distance(output_headers.begin(), class_idx_it);
        // add to confusion matrix row in proportion
        output_values[class_idx] +=
            1.f / static_cast<float>(img_objects.size());
      }
    }
    // check break conditions if breaking on certain level of recall
    if (cond == CV_VOC_CCOND_RECALL) {
      if (static_cast<float>(retrieved_hits) /
  ...
  for (vector<float>::iterator it = output_values.begin();
       it < output_values.end(); ++it) {
    (*it) /= static_cast<float>(total_images);
  }
}

// NOTE: doesn't ignore repeated detections
void VocData::calcDetectorConfMatRow(
    const string& obj_class, const ObdDatasetType dataset,
    const vector<ObdImage>& images, const vector<vector<float> >& scores,
    const vector<vector<Rect> >& bounding_boxes, const VocConfCond cond,
    const float threshold, vector<string>& output_headers,
    vector<float>& output_values, bool ignore_difficult) {
  CV_Assert(images.size() == scores.size());
  CV_Assert(images.size() == bounding_boxes.size());
  ...
  if (cond == CV_VOC_CCOND_RECALL) {
    //        vector<char> ground_truth;
    //        /* in order to calculate the total number of relevant images for
    //        normalization of recall
    //            it's necessary to extract the ground truth for the images
    //            under consideration */
    //        getClassifierGroundTruth(obj_class, images, ground_truth);
    //        total_relevant =
    //        std::count_if(ground_truth.begin(),ground_truth.end(),std::bind2nd(std::equal_to<bool>(),true));
    /* calculate the total number of objects in the ground truth for the current
     * dataset */
    vector<ObdImage> gt_images;
    vector<char> gt_object_present;
  ...
  for (size_t image_idx = 0; image_idx < images.size(); ++image_idx) {
    ...
    // find the ground truth object which has the highest overlap score with the
    // detected object
    float maxov = -1.0;
    int max_gt_obj_idx = -1;
    //-- for each detected object iterate through objects present in ground
    // truth --
    for (size_t gt_obj_idx = 0; gt_obj_idx < img_objects.size(); ++gt_obj_idx) {
      // check difficulty flag
      if (ignore_difficult ||
          (img_object_data[gt_obj_idx].difficult == false)) {
        // if the class matches, then check if the detected object and ground
        // truth object overlap by a sufficient margin
        float ov =
            testBoundingBoxesForOverlap(bounding_boxes_flat[ranking[image_idx]],
                                        img_objects[gt_obj_idx].boundingBox);
        if (ov != -1.f) {
          // if all conditions are met store the overlap score and index (as
          // objects are assigned to the highest scoring match)
          if (ov > maxov) {
    ...
    if (maxov != -1.0) {
      ...
      vector<string>::iterator class_idx_it =
          std::find(output_headers.begin(), output_headers.end(),
                    img_objects[max_gt_obj_idx].object_class);
      // if the class name extracted from the ground truth file could not be
      // found in the list of available classes, raise an exception
      if (class_idx_it == output_headers.end()) {
        string err_msg = "could not find object class '" +
...
//--------------------------------------------------------------
// INPUTS:
// - output_file        The file to which to save the GNUPlot data file. If only
// a filename is specified, the data
//                      file is saved to the standard VOC results directory.
// - precision          Vector of precisions as returned from
// calcClassifier/DetectorPrecRecall
// - recall             Vector of recalls as returned from
// calcClassifier/DetectorPrecRecall
// - ap                ap as returned from calcClassifier/DetectorPrecRecall
// - (title)            Title to use for the plot (if not specified, just the ap
// is printed as the title)
//                      This also specifies the filename of the output file if
//                      printing to pdf
// - (plot_type)        Specifies whether to instruct GNUPlot to save to a PDF
// file (CV_VOC_PLOT_PDF) or directly
//                      to screen (CV_VOC_PLOT_SCREEN) in the datafile
// NOTES:
// The GNUPlot data file can be executed using GNUPlot from the commandline in
// the following way:
//      >> GNUPlot <output_file>
// This will then display the p-r curve on the screen or save it to a pdf file
// depending on plot_type

void VocData::savePrecRecallToGnuplot(const string& output_file,
                                      const vector<float>& precision,
                                      const vector<float>& recall,
                                      const float ap, const string title,
                                      const VocPlotType plot_type) {
  string output_file_std = checkFilenamePathsep(output_file);

  // if no directory is specified, by default save the output file in the
  // results directory
  ...
  if (plot_file.is_open()) {
    ...
    plot_file << "set title \"" << title_text << " (ap: " << ap << ")\""
              << endl;
    ...
    string err_msg =
        "could not open plot file '" + output_file_std + "' for writing.";
    CV_Error(Error::StsError, err_msg.c_str());
  }
}

void VocData::readClassifierGroundTruth(const string& obj_class,
                                        const ObdDatasetType dataset,
                                        vector<ObdImage>& images,
                                        vector<char>& object_present) {
  images.clear();
...
void VocData::readClassifierResultsFile(const std::string& input_file,
                                        vector<ObdImage>& images,
                                        vector<float>& scores) {
  images.clear();
  ...
  // if no directory is specified, by default search for the input file in the
  // results directory
...
void VocData::readDetectorResultsFile(const string& input_file,
                                      vector<ObdImage>& images,
                                      vector<vector<float> >& scores,
                                      vector<vector<Rect> >& bounding_boxes) {
  images.clear();
...
const vector<string>& VocData::getObjectClasses() { return m_object_classes; }
...
static string getVocName(const string& vocPath) {
  ...
  if (found == string::npos) {
    found = vocPath.rfind('\\');
    if (found == string::npos) return vocPath;
  }
...
//-----------------------------------------------------------
void VocData::initVoc2007to2010(const string& vocPath,
                                const bool useTestDataset) {
  ...
  // up until VOC 2010, ap was calculated by sampling p-r curve, not taking
  // complete curve
  m_sampled_ap = ((m_vocName == "VOC2007") || (m_vocName == "VOC2008") ||
                  (m_vocName == "VOC2009"));
}

// Read a VOC classification ground truth text file for a given object class and
// dataset
//-------------------------------------------------------------------------------------
// INPUTS:
// - filename           The path of the text file to read
// OUTPUTS:
// - image_codes        VOC image codes extracted from the GT file in the form
// 20XX_XXXXXX where the first four
//                          digits specify the year of the dataset, and the last
//                          group specifies a unique ID
// - object_present     For each image in the 'image_codes' array, specifies
// whether the object class described
//                          in the loaded GT file is present or not
void VocData::readClassifierGroundTruth(const string& filename,
                                        vector<string>& image_codes,
                                        vector<char>& object_present) {
  image_codes.clear();
  ...
  if (!gtfile.is_open()) {
    string err_msg =
        "could not open VOC ground truth textfile '" + filename + "'.";
    CV_Error(Error::StsError, err_msg.c_str());
  }
    ...
    if (!iss.fail()) {
      ...
      if (!gtfile.eof())
        CV_Error(Error::StsParseError,
                 "error parsing VOC ground truth textfile.");
    }
  }
  gtfile.close();
}

void VocData::readClassifierResultsFile(const string& input_file,
                                        vector<string>& image_codes,
                                        vector<float>& scores) {
  ...
  if (result_file.is_open()) {
    ...
    while (!result_file.eof()) {
      ...
      if (!iss.fail()) {
        ...
        if (!result_file.eof())
          CV_Error(Error::StsParseError,
                   "error parsing VOC classifier results file.");
      }
    }
    result_file.close();
  } else {
    string err_msg = "could not open classifier results file '" + input_file +
                     "' for reading.";
    CV_Error(Error::StsError, err_msg.c_str());
  }
}
...
void VocData::readDetectorResultsFile(const string& input_file,
  ...
  if (result_file.is_open()) {
    ...
    while (!result_file.eof()) {
      std::getline(result_file, line);
      std::istringstream iss(line);
      iss >> image >> score >> bounding_box.x >> bounding_box.y >>
      ...
      if (!iss.fail()) {
        ...
        vector<string>::iterator image_codes_it =
            std::find(image_codes.begin(), image_codes.end(), image);
        if (image_codes_it == image_codes.end()) {
          ...
          /* if the image index has been seen before, add the current object
           * below it in the 2D arrays */
          int image_idx =
              (int)std::distance(image_codes.begin(), image_codes_it);
        ...
        if (!result_file.eof())
          CV_Error(Error::StsParseError,
                   "error parsing VOC detector results file.");
      }
    }
    result_file.close();
  } else {
    string err_msg = "could not open detector results file '" + input_file +
                     "' for reading.";
    CV_Error(Error::StsError, err_msg.c_str());
  }
}
...
// OUTPUTS:
// - objects            Array of VocObject describing all object instances
// present in the given image
void VocData::extractVocObjects(const string filename,
                                vector<ObdObject>& objects,
                                vector<VocObjectData>& object_data) {
#ifdef PR_DEBUG
  int block = 1;
  ...
  if (extractXMLBlock(contents, "annotation", 0, contents) != -1) {
    ...
    while (searchpos != -1) {
      ...
      if (extractXMLBlock(object_contents, "name", 0, tag_contents) == -1)
        CV_Error(
            Error::StsError,
            "missing <name> tag in object definition of '" + filename + "'");
      ...
      if (extractXMLBlock(object_contents, "difficult", 0, tag_contents) !=
          -1) {
      ...
      if (extractXMLBlock(object_contents, "truncated", 0, tag_contents) !=
          -1) {
      ...
      searchpos =
          extractXMLBlock(contents, "object", searchpos, object_contents);
    }
  }
}

// Converts an image identifier string in the format YYYY_XXXXXX to a single
// index integer of form XXXXXXYYYY where Y represents a year and returns the
// image path
//----------------------------------------------------------------------------------------------------------
...
// Tests two boundary boxes for overlap (using the intersection over union
// metric) and returns the overlap if the objects defined by the two bounding
// boxes are considered to be matched according to the criterion outlined in the
// VOC documentation [namely intersection/union > some threshold] otherwise
// returns -1.0 (no match)
//----------------------------------------------------------------------------------------------------------
float VocData::testBoundingBoxesForOverlap(const Rect detection,
                                           const Rect ground_truth) {
  ...
  int intersection_x2 =
      std::min(detection_x2, ground_truth_x2);  // leftmost right
  int intersection_y2 =
      std::min(detection_y2, ground_truth_y2);  // topmost bottom
  ...
  int union_area = (detection.width + 1) * (detection.height + 1) +
                   (ground_truth.width + 1) * (ground_truth.height + 1) -
                   intersection_area;

  // calculate the intersection over union and use as threshold as per VOC
  // documentation
  float overlap =
      static_cast<float>(intersection_area) / static_cast<float>(union_area);
...
// Extracts the object class and dataset from the filename of a VOC standard
// results text file, which takes the format
// 'comp<n>_{cls/det}_<dataset>_<objclass>.txt'
//----------------------------------------------------------------------------------------------------------
void VocData::extractDataFromResultsFilename(const string& input_file,
                                             string& class_name,
                                             string& dataset_name) {
  string input_file_std = checkFilenamePathsep(input_file);
  ...
  // extract dataset and class names, triggering exception if the filename
  ...
  // allow for appended index after a further '_' by discarding this part if it
  // exists
  ...
  if ((datasetstart == filename.npos) || (classstart == filename.npos))
    CV_Error(Error::StsError,
             "Error parsing results filename. Is it in standard format of "
             "'comp<n>_{cls/det}_<dataset>_<objclass>.txt'?");
  ++datasetstart;
  ++classstart;
...
bool VocData::getClassifierGroundTruthImage(const string& obj_class,
                                            const string& id) {
  /* if the classifier ground truth data for all images of the current class has
   * not been loaded yet, load it now */
  if (m_classifier_gt_all_ids.empty() || (m_classifier_gt_class != obj_class)) {
    ...
    for (int i = 0; i < 2;
         ++i)  // run twice (once over test set and once over training set)
    {
      // generate the filename of the classification ground-truth textfile for
      // the object class
      string gtFilename = m_class_imageset_path;
      gtFilename.replace(gtFilename.find("%s"), 2, obj_class);
      if (i == 0) {
      ...
      m_classifier_gt_all_ids.insert(m_classifier_gt_all_ids.end(),
                                     image_codes.begin(), image_codes.end());
      m_classifier_gt_all_present.insert(m_classifier_gt_all_present.end(),
                                         object_present.begin(),
                                         object_present.end());

      CV_Assert(m_classifier_gt_all_ids.size() ==
                m_classifier_gt_all_present.size());
    }
  }

  // search for the image code
  vector<string>::iterator it =
      find(m_classifier_gt_all_ids.begin(), m_classifier_gt_all_ids.end(), id);
  if (it != m_classifier_gt_all_ids.end()) {
    // image found, so return corresponding ground truth
    return m_classifier_gt_all_present[std::distance(
               m_classifier_gt_all_ids.begin(), it)] != 0;
  }
  string err_msg = "could not find classifier ground truth for image '" + id +
                   "' and class '" + obj_class + "'";
  CV_Error(Error::StsError, err_msg.c_str());
}
...
// returns a vector containing indexes of the input vector in sorted
// ascending/descending order
void VocData::getSortOrder(const vector<float>& values, vector<size_t>& order,
                           bool descending) {
  /* 1. store sorting order in 'order_pair' */
  vector<std::pair<size_t, vector<float>::const_iterator> > order_pair(
      values.size());

  size_t n = 0;
  for (vector<float>::const_iterator it = values.begin(); it != values.end();
       ++it, ++n)
...
int VocData::stringToInteger(const string input_str) {
  ...
  if ((ss >> result).fail()) {
    CV_Error(Error::StsBadArg,
             "could not perform string to integer conversion");
  }
  return result;
}
...
string VocData::integerToString(const int input_int) {
  ...
  if ((ss << input_int).fail()) {
    CV_Error(Error::StsBadArg,
             "could not perform integer to string conversion");
  }
  result = ss.str();
  return result;
}

string VocData::checkFilenamePathsep(const string filename,
...
void VocData::convertImageCodesToObdImages(const vector<string>& image_codes,
                                           vector<ObdImage>& images) {
  images.clear();
...
//      (can be used to call extractXMLBlock recursively to extract multiple
//      blocks) returns -1 if the tag could not be found
int VocData::extractXMLBlock(const string src, const string tag,
                             const int searchpos, string& tag_contents) {
  ...
  while (embed_count > 0) {
    ...
    // the next code is only executed if there are embedded tags with the same
    // name
...
struct DDMParams {
  ...
  DDMParams(const string _detectorType, const string _descriptorType,
            const string& _matcherType)
...
struct VocabTrainParams {
  VocabTrainParams()
      : trainObjClass("chair"),
        ...
        memoryUse(200),
        descProportion(0.3f) {}
  VocabTrainParams(const string _trainObjClass, size_t _vocabSize,
  ...
  string trainObjClass;  // Object class used for training visual vocabulary.
                         // It shouldn't matter which object class is specified
                         // here - visual vocab will still be the same.
  int vocabSize;         // number of visual words in vocabulary to train
  int memoryUse;         // Memory to preallocate (in MB) when training vocab.
                  // Change this depending on the size of the dataset/available
                  // memory.
  float descProportion;  // Specifies the number of descriptors to use from each
                         // image as a proportion of the total num descs.
};

struct SVMTrainParamsExt {
  SVMTrainParamsExt()
      : descPercent(0.5f), targetRatio(0.4f), balanceClasses(true) {}
  ...
  float
      descPercent;  // Percentage of extracted descriptors to use for training.
...
static void readUsedParams(const FileNode& fn, string& vocName,
                           DDMParams& ddmParams,
...
static void writeUsedParams(FileStorage& fs, const string& vocName,
                            const DDMParams& ddmParams,
                            const VocabTrainParams& vocabTrainParams,
...
static Mat trainVocabulary(const string& filename, VocData& vocData,
                           const VocabTrainParams& trainParams,
  ...
  if (!readVocabulary(filename, vocabulary)) {
    ...
    BOWKMeansTrainer bowTrainer(trainParams.vocabSize, terminate_criterion, 3,
                                KMEANS_PP_CENTERS);

    while (images.size() > 0) {
      if (bowTrainer.descriptorsCount() > maxDescCount) {
#ifdef DEBUG_DESC_PROGRESS
        cout << "Breaking due to full memory ( descriptors count = "
             << bowTrainer.descriptorsCount()
             << "; descriptor size in bytes = " << descByteSize
             << "; all used memory = "
      ...
      if (!imageDescriptors.empty()) {
        ...
        int descsToExtract = static_cast<int>(trainParams.descProportion *
                                              static_cast<float>(descCount));
...
#ifdef DEBUG_DESC_PROGRESS
      cout << images.size() << " images left, " << images[randImgIdx].id
           << " processed - "
           << /* descs_extracted << "/" << image_descriptors.rows << " extracted
                 - " << */
          cvRound((static_cast<double>(bowTrainer.descriptorsCount()) /
    ...
    cout << "Maximum allowed descriptor count: " << maxDescCount
    ...
    if (!writeVocabulary(filename, vocabulary)) {
      cout << "Error: file " << filename << " can not be opened to write"
           << endl;
      exit(-1);
    }
  }
...
static bool readBowImageDescriptor(const string& file,
                                   Mat& bowImageDescriptor) {
...
static bool writeBowImageDescriptor(const string& file,
                                    const Mat& bowImageDescriptor) {
...
static void calculateImageDescriptors(
    const vector<ObdImage>& images, vector<Mat>& imageDescriptors,
    Ptr<BOWImgDescriptorExtractor>& bowExtractor,
    const Ptr<FeatureDetector>& fdetector, const string& resPath) {
  CV_Assert(!bowExtractor->getVocabulary().empty());
  imageDescriptors.resize(images.size());

  for (size_t i = 0; i < images.size(); i++) {
    string filename =
        resPath + bowImageDescriptorsDir + "/" + images[i].id + ".xml.gz";
    if (readBowImageDescriptor(filename, imageDescriptors[i])) {
#ifdef DEBUG_DESC_PROGRESS
      cout << "Loaded bag of word vector for image " << i + 1 << " of "
           << images.size() << " (" << images[i].id << ")" << endl;
#endif
    } else {
      Mat colorImage = imread(images[i].path);
#ifdef DEBUG_DESC_PROGRESS
      cout << "Computing descriptors for image " << i + 1 << " of "
           << images.size() << " (" << images[i].id << ")" << flush;
#endif
...
#ifdef DEBUG_DESC_PROGRESS
      cout << " ...DONE "
           << static_cast<int>(static_cast<float>(i + 1) /
                               static_cast<float>(images.size()) * 100.0)
           << " % complete" << endl;
#endif
      if (!imageDescriptors[i].empty()) {
        if (!writeBowImageDescriptor(filename, imageDescriptors[i])) {
          cout << "Error: file " << filename
               << "can not be opened to write bow image descriptor" << endl;
          exit(-1);
        }
      }
    }
...
static void removeEmptyBowImageDescriptors(vector<ObdImage>& images,
  ...
  for (int i = (int)images.size() - 1; i >= 0; i--) {
    ...
    if (res) {
      cout << "Removing image " << images[i].id << " due to no descriptors..."
           << endl;
...
static void removeBowImageDescriptorsByCount(
    vector<ObdImage>& images, vector<Mat> bowImageDescriptors,
    vector<char> objectPresent, const SVMTrainParamsExt& svmParamsExt,
    int descsToDelete) {
  RNG& rng = theRNG();
  int pos_ex =
      (int)std::count(objectPresent.begin(), objectPresent.end(), (char)1);
  int neg_ex =
      (int)std::count(objectPresent.begin(), objectPresent.end(), (char)0);

  while (descsToDelete != 0) {
    int randIdx = rng((unsigned)images.size());

    // Prefer positive training examples according to svmParamsExt.targetRatio
    // if required
    if (objectPresent[randIdx]) {
      if ((static_cast<float>(pos_ex) / static_cast<float>(neg_ex + pos_ex) <
           svmParamsExt.targetRatio) &&
...
static void setSVMParams(Ptr<SVM>& svm, const Mat& responses,
                         bool balanceClasses) {
  int pos_ex = countNonZero(responses == 1);
  int neg_ex = countNonZero(responses == -1);
  cout << pos_ex << " positive training samples; " << neg_ex
       << " negative training samples" << endl;
...
static Ptr<SVM> trainSVMClassifier(const SVMTrainParamsExt& svmParamsExt,
                                   const string& objClassName, VocData& vocData,
                                   Ptr<BOWImgDescriptorExtractor>& bowExtractor,
                                   const Ptr<FeatureDetector>& fdetector,
                                   const string& resPath) {
  /* first check if a previously trained svm for the current class has been
   * saved to file */
  ...
  if (fs.isOpened()) {
    ...
    CV_Assert(svmParamsExt.descPercent > 0.f &&
              svmParamsExt.descPercent <= 1.f);
    if (svmParamsExt.descPercent < 1.f) {
      ...
      cout << "Using " << (images.size() - descsToDelete) << " of "
           << images.size() << " descriptors for training ("
           << svmParamsExt.descPercent * 100.0 << " %)" << endl;
      removeBowImageDescriptorsByCount(images, bowImageDescriptors,
                                       objectPresent, svmParamsExt,
                                       descsToDelete);
    }

    // Prepare the input matrices for SVM training.
    Mat trainData((int)images.size(), bowExtractor->getVocabulary().rows,
                  CV_32FC1);
    ...
    setSVMTrainAutoParams(c_grid, gamma_grid, p_grid, nu_grid, coef_grid,
                          degree_grid);
...
static void computeConfidences(const Ptr<SVM>& svm, const string& objClassName,
                               ...
                               const Ptr<FeatureDetector>& fdetector,
                               const string& resPath) {
  cout << "*** CALCULATING CONFIDENCES FOR CLASS " << objClassName << " ***"
       << endl;
  cout << "CALCULATING BOW VECTORS FOR TEST SET OF " << objClassName << "..."
       << endl;
  ...
  // in test set
  cout << "CALCULATING CONFIDENCE SCORES FOR CLASS " << objClassName << "..."
       << endl;
  ...
  for (size_t imageIdx = 0; imageIdx < images.size(); imageIdx++) {
    if (imageIdx == 0) {
      ...
      float classVal = confidences[imageIdx] =
          svm->predict(bowImageDescriptors[imageIdx], noArray(), 0);
      float scoreVal = confidences[imageIdx] = svm->predict(
          bowImageDescriptors[imageIdx], noArray(), StatModel::RAW_OUTPUT);
      signMul = (classVal < 0) == (scoreVal < 0) ? 1.f : -1.f;
    }
    // svm output of decision function
    confidences[imageIdx] =
        signMul * svm->predict(bowImageDescriptors[imageIdx], noArray(),
                               StatModel::RAW_OUTPUT);
  }

  cout << "WRITING QUERY RESULTS TO VOC RESULTS FILE FOR CLASS " << objClassName
       << "..." << endl;
  vocData.writeClassifierResultsFile(resPath + plotsDir, objClassName,
                                     CV_OBD_TEST, images, confidences, 1, true);
...
static void computeGnuPlotOutput(const string& resPath,
                                 const string& objClassName, VocData& vocData) {
  vector<float> precision, recall;
  float ap;

  const string resultFile = vocData.getResultsFilename(
      objClassName, CV_VOC_TASK_CLASSIFICATION, CV_OBD_TEST);
  const string plotFile = resultFile.substr(0, resultFile.size() - 4) + ".plt";

  cout << "Calculating precision recall curve for class '" << objClassName
       << "'" << endl;
  vocData.calcClassifierPrecRecall(resPath + plotsDir + "/" + resultFile,
                                   precision, recall, ap, true);
  cout << "Outputting to GNUPlot file..." << endl;
  vocData.savePrecRecallToGnuplot(resPath + plotsDir + "/" + plotFile,
                                  precision, recall, ap, objClassName,
                                  CV_VOC_PLOT_PNG);
}

static Ptr<Feature2D> createByName(const String& name) {
  // Available detectors (without contrib):
  // ORB, BRISK, AKAZE, MSER,
  // FastFeatureDetector, AgastFeatureDetector,
  // GFTTDetector, SimpleBlobDetector

  // Available detectors in contrib:
  // StarDetector, HarrisLaplaceFeatureDetector, AffineFeature2D
  // SURF(nonfree)

  // Available descriptors from contrib:
  // FREAK, BriefDescriptorExtractor, LUCID (color only),
  // LATCH, DAISY, MSDDetector, VGG, BoostDesc, AffineFeature2D

  // Note that combined detector/descriptor should be used
  // with float type descriptors
  // The only suitable is KAZE and SIFT
...
int main(int argc, char** argv) {
  ...
  if (paramsFS.isOpened()) {
    readUsedParams(paramsFS.root(), vocName, ddmParams, vocabTrainParams,
                   svmTrainParamsExt);
    CV_Assert(vocName == getVocName(vocPath));
  } else {
    vocName = getVocName(vocPath);
    if (argc != 6) {
      cout << "Feature detector, descriptor extractor, descriptor matcher must "
              "be set"
           << endl;
      return -1;
    }
    ...
    if (paramsFS.isOpened()) {
      writeUsedParams(paramsFS, vocName, ddmParams, vocabTrainParams,
                      svmTrainParamsExt);
      paramsFS.release();
    } else {
      cout << "File " << (resPath + "/" + paramsFile)
           << "can not be opened to write" << endl;
      return -1;
    }
  }
  ...
  }
  {
    Ptr<DescriptorMatcher> descMatcher =
        DescriptorMatcher::create(ddmParams.matcherType);
    if (!featureDetector || !descExtractor || !descMatcher) {
      cout << "descMatcher was not created" << endl;
      return -1;
    }
    bowExtractor =
        makePtr<BOWImgDescriptorExtractor>(descExtractor, descMatcher);
  }

  // Print configuration to screen
  printUsedParams(vocPath, resPath, ddmParams, vocabTrainParams,
                  svmTrainParamsExt);
  // Create object to work with VOC
  VocData vocData(vocPath, false);

  // 1. Train visual word vocabulary if a pre-calculated vocabulary file doesn't
  // already exist from previous run
  Mat vocabulary =
      trainVocabulary(resPath + "/" + vocabularyFile, vocData, vocabTrainParams,
                      featureDetector, descExtractor);
  bowExtractor->setVocabulary(vocabulary);

  // 2. Train a classifier and run a sample query for each object class
  const vector<string>& objClasses =
      vocData.getObjectClasses();  // object class list
  for (size_t classIdx = 0; classIdx < objClasses.size(); ++classIdx) {
    // Train a classifier on train dataset
    Ptr<SVM> svm =
        trainSVMClassifier(svmTrainParamsExt, objClasses[classIdx], vocData,
    ...
    computeConfidences(svm, objClasses[classIdx], vocData, bowExtractor,
                       featureDetector, resPath);
